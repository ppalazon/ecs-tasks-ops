...
def get_ecs_list(operation_name, response_attribute, **operation_params):
    ...
    return list(chain.from_iterable([resp.get(response_attribute, []) for resp in paginator_ite]))
...
def get_describe_services(cluster_name, services_arns):
    return ecs_client.describe_services(cluster=cluster_name, services=services_arns).get('services', [])


def get_describe_tasks(cluster_name, tasks_arns):
    return ecs_client.describe_tasks(cluster=cluster_name, tasks=tasks_arns).get('tasks', [])


def get_describe_container_instances(cluster_name, container_instances_arns):
    return ecs_client.describe_container_instances(cluster=cluster_name, containerInstances=container_instances_arns).get('containerInstances', [])


def get_all_services(cluster_name):
    """Get information about all services defined in a cluster."""
    list_services_arns = get_ecs_list('list_services', 'serviceArns', cluster=cluster_name)
    ...
    services = list(chain.from_iterable([get_describe_services(cluster_name, list_services_arns[slc]) for slc in slices]))
    return services


def get_all_container_instances(cluster_name):
    """Get information about all container instances defined in a cluster."""
    list_container_instances_arns = get_ecs_list('list_container_instances', 'containerInstanceArns', cluster=cluster_name)
    # Max request per describe_services are 10 services arns, so we slice them 100 by 100, and request each 100
    slices = [slice(i, i+100, 1) for i in range(0, len(list_container_instances_arns), 100)]
    containers_instances = list(chain.from_iterable([get_describe_container_instances(cluster_name, list_container_instances_arns[slc]) for slc in slices]))
...
def restart_service(cluster_name, service_arn, force_new_deployment=False):
    """Stop a specific task"""
    return ecs_client.update_service(cluster=cluster_name, service=service_arn, forceNewDeployment=force_new_deployment).get('service', {})